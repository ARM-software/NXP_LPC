 ...
 * $Revision:    V1.1
 *
 * Project:      GPDMA Driver for NXP LPC40xx
 * -------------------------------------------------------------------------- */
...
#define LPC40xx_GPDMA     ((GPDMA_REG         *) LPC_GPDMA_BASE)
...
int32_t GPDMA_Initialize (void) {
  ...
  LPC40xx_GPDMA->DMACIntTCClear = 0xFFU;
  LPC40xx_GPDMA->DMACIntErrClr  = 0xFFU;
...
int32_t GPDMA_PeripheralSelect (uint8_t peri, uint8_t sel) {
  ...
  LPC_SC->DMAREQSEL = (sel) ? (LPC_SC->DMAREQSEL |  (1U << peri)) :\
                              (LPC_SC->DMAREQSEL & ~(1U << peri));
  return 0;
}
...
int32_t GPDMA_ChannelConfigure (uint8_t              ch,
  ...
  LPC40xx_GPDMA->DMACIntTCClear = (1U << ch);
  LPC40xx_GPDMA->DMACIntErrClr  = (1U << ch);

  // Link list not supported
  dma_ch->LLI = 0U;

  // Enable DMA Channels, little endian
  LPC40xx_GPDMA->DMACConfig = GPDMA_CONFIG_E;
  while ((LPC40xx_GPDMA->DMACConfig & GPDMA_CONFIG_E) == 0U);
...
void DMA_IRQHandler (void) {
  ...
  for (ch = 0U; ch < GPDMA_NUMBER_OF_CHANNELS; ch++) {
    if (LPC40xx_GPDMA->DMACIntStat & (1U << ch)) {
      dma_ch = GPDMA_CHANNEL(ch);

      // Terminal count request interrupt
      if (LPC40xx_GPDMA->DMACIntTCStat & (1U << ch)) {
        ...
        // DMA error interrupt
        if (LPC40xx_GPDMA->DMACIntErrStat & (1U << ch)) {
          dma_ch->CONFIG  = 0U;
          dma_ch->CONTROL = 0U;
