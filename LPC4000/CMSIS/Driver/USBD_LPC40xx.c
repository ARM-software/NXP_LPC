 ...
 * Project:      USB Device Driver for NXP LPC40xx
 * --------------------------------------------------------------------------
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      if ((usb_state & 0xF0U) == 0U) {                  // If Host is not enabled
        LPC_USB->OTGClkCtrl &= ~0x02U;                  // Disable OTG Device clock
        while ((LPC_USB->OTGClkSt & 2U) != 0U);
        LPC_USB->USBClkCtrl &= ~0x10U;                  // Disable AHB clock
        while ((LPC_USB->USBClkSt & 0x10U) != 0U);
        LPC_USB->OTGClkCtrl &= ~0x18U;                  // Disable AHB and OTG clocks
        while ((LPC_USB->OTGClkSt & 0x18U) != 0U);
