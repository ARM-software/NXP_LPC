 ...
 * Project:      CAN Driver for NXP LPC43xx
 * --------------------------------------------------------------------------
...
static int32_t CANx_PowerControl (ARM_POWER_STATE state, uint8_t x) {
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      if (x == 0U) {
        NVIC_DisableIRQ (MX_C_CAN0_IRQn);
    ...
    case ARM_POWER_FULL:
      ...
      if (x == 0U) {
        NVIC_ClearPendingIRQ (MX_C_CAN0_IRQn);
        NVIC_EnableIRQ       (MX_C_CAN0_IRQn);
      } else {
...
/**
  \fn          void MX_C_CAN0_IRQHandler (void)
...
/**
  \fn          void MX_C_CAN1_IRQHandler (void)
