 ...
 * Project:      Ethernet Media Access (MAC) Driver for NXP LPC43xx
 * --------------------------------------------------------------------------
...
#include "EMAC_LPC43xx.h"

#define ARM_ETH_MAC_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,8) /* driver version */
...
static int32_t PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    case ARM_POWER_OFF:
      ...
      LPC_RGU->RESET_CTRL0 = (RGU_RESET_EMAC | (~(LPC_RGU->RESET_ACTIVE_STATUS0)));
      while (!(LPC_RGU->RESET_ACTIVE_STATUS0 & RGU_RESET_EMAC));

      /* Disable EMAC peripheral clock */
      LPC_CCU1->CLK_M4_ETHERNET_CFG &= ~CCU_CLK_CFG_RUN;
    ...
    case ARM_POWER_FULL:
      ...
      LPC_CCU1->CLK_M4_ETHERNET_CFG |=  CCU_CLK_CFG_AUTO | CCU_CLK_CFG_RUN;
      while (!(LPC_CCU1->CLK_M4_ETHERNET_STAT & CCU_CLK_STAT_RUN));
      ...
      NVIC_ClearPendingIRQ(MX_ETHERNET_IRQn);
      NVIC_EnableIRQ(MX_ETHERNET_IRQn);
...
*/
void MX_ETHERNET_IRQHandler (void) {
